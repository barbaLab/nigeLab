...
o = nigeLab.utils.LinePlotReducer(x');
lpe = nigeLab.utils.LinePlotExplorer(gcf());

%% plot
% Pass |plot| into the LPR and specify x and y values.
lpr = nigeLab.utils.LinePlotReducer(@plot, t', x');
lpe = nigeLab.utils.LinePlotExplorer(gcf(), 0, 100);

%% plot
% Try it with matrix inputs.
lpr = nigeLab.utils.LinePlotReducer(@plot, [t' t' t'], x');

%% plot
% See if it handles linespec values gracefully.
lpr = nigeLab.utils.LinePlotReducer(t, x(1, :), 'b:', ...
                      t, x(2, :), 'g', ...
                      t, x(3, :), 'r--*');

%% plot
% What about a matrix with a linespec?
o = nigeLab.utils.LinePlotReducer(@plot, t, x, ':');

%% plot
% Lots of data with no linespec?
o = nigeLab.utils.LinePlotReducer(@plot, t, x(1, :), t, x(2, :), t, x(3, :));

%% semilogx
% What about other plot options, like semilogx.
o = nigeLab.utils.LinePlotReducer(@semilogx, t, abs(x));

%% semilogy
% ... and semilogy (this might look a little funny).
o = nigeLab.utils.LinePlotReducer(@semilogy, t, abs(x));
...
% different size).
o = nigeLab.utils.LinePlotReducer(@stairs, t, x(1, :), 'b:');
...
try
    o = nigeLab.utils.LinePlotReducer(@stairs, t, x, 'b:');
catch er
...
o = nigeLab.utils.LinePlotReducer(@plotyy, t, x(1, :), t, x(2, :));
lpe = nigeLab.utils.LinePlotExplorer(gcf(), 0, 100);
...
h1 = subplot(2, 1, 1);
nigeLab.utils.LinePlotReducer(t, x);
h2 = subplot(2, 1, 2);
nigeLab.utils.LinePlotReducer(t, 1-2*x);
...
clf();
h = plot(t, x(1, :));
lpr = nigeLab.utils.LinePlotReducer(h);
...
lpr = nigeLab.utils.LinePlotReducer(h, t, x);
lpe = nigeLab.utils.LinePlotExplorer();
...
nigeLab.utils.reduce_plot(x(1, :).', 'r');
...
clf();
hold on;
nigeLab.utils.LinePlotReducer(t, x(1, :), 'r');
nigeLab.utils.LinePlotReducer(t, x(2, :), 'b');
nigeLab.utils.LinePlotReducer(t, x(3, :), 'g');
nigeLab.utils.LinePlotExplorer();
hold off;
