function count = cprintf(style,verbose,format,varargin)
...
%
% Change log:
%    2020-01-31: Ported from version as provided by YMA (add "package" call
%                 for nigeLab and optional third input flag to make it work
%                 together with `Verbose` flags directly for nigeLab
%                 application purposes) - MM
...
if ischar(verbose)
   % Then `"format"` is actually first varargin
   varargin = [format, varargin];
   format = verbose; % verbose was the format
   verbose = true; % we didn't specify verbose, so it is true
end
...
if ~exist('el','var')
   el=handle([]);
elseif isempty(el) %#ok<NODEF>
   el = handle([]);
end
if nargin<1
   showDemo(majorVersion,minorVersion);
   return;
end
if isempty(style)
   return;
