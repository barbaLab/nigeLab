...
for iCh = chan
   ...
   for bb=1:numel(animalObj.Blocks)
      ...
      SuppressText = true;
      classes_ = blockObj.getSort(iCh,SuppressText);
      
      if ~ischar(unit)
         ind = find(ismember(classes_,unit));
      else
         ind = 1:numel(classes_);
      end
      ...
      [temp,classes] = SPCrun(par,inspk_aux);
      ...
      for bb=1:numel(animalObj.Blocks)
         blockObj = animalObj.Blocks(bb);
         saveSorted(blockObj,classes(BlInd == bb),temp,iCh);         
...
function saveSorted(blockObj,classes,temp,iCh)
