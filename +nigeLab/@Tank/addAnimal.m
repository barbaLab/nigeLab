...
for i = 1:numel(animalObj)
   animalObj(i).Listener = [animalObj(i).Listener, ...
      addlistener(animalObj(i),'ObjectBeingDestroyed',...
         @(~,~)tankObj.AssignNULL)];
   tankObj.AnimalListener = [tankObj.AnimalListener, ...
      addlistener(animalObj(i),'StatusChanged',...
         @(~,evt)notify(tankObj,'StatusChanged',evt))];
end
