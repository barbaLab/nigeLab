function [xPoints, yPoints] = CPL_plotSpikeRaster(spikes,varargin)
...
%
%       Extra - default {NaN}
%           Can be specified as M x k cell matrix, where M is number of
%           trials and k is the number of additional "non-spike" events to
%           add as different colored lines. [NOTE: currently only works for
%           'vertline' for M x 1 cell array of times format]. Each cell
%           element is a series of times just like for spikes, but that
%           correspond to behavioral or other experimental events (for
%           example { M x 1 column of cells for times of reach onsets
%           occurring within epochs near grasps, M x 1 column of times
%           support arm was raised during epochs around grasps})
%
%       ExtraLabel - default {NaN}
%           Can be specified as k x 1 cell matrix, where k is the number of 
%           additional "non-spike" events to add as colored lines. Each
%           cell element is a character label corresponding to the event
%           type (for example {'reach'; 'support}).
%
...
%%
%% MODIFICATIONS  :  Max Murphy (R2017b)
...
p.addParamValue('Extra',{NaN},@(x) iscell(x));
p.addParamValue('ExtraLabel',{NaN},@(x) iscell(x));
p.addParamValue('PlotHandle',gca,@isobject);
p.addParamValue('FigHandle',gcf,@isobject);
...
useExtra = ~any(isnan(extra{1}));
useExtraLabel = (~any(isnan(extra{1}))) && (numel(extraLabel) == size(extra,2));
...
if islogical(spikes)
   ...
   if strcmpi(plotType,'vertline') || strcmpi(plotType,'horzline')
      ...
      if useExtra
         ex_xPoints = cell(size(extra,2),1);
         ex_yPoints = cell(size(extra,2),1);
         ex_currentInd = ones(1,size(extra,2));
         ex_nTotal = zeros(1,size(extra,2));
         for iK = 1:size(extra,2)
            ex_nTotal(iK) = sum(cellfun(@length,extra(:,iK)));
            ex_xPoints{iK} = NaN(ex_nTotal(iK)*3,1);
            ex_yPoints{iK} = ex_xPoints{iK};
         end
      end
      ...
      if strcmpi(plotType,'vertline')
         ...
         for trials = 1:nTrials
            ...
            % Add possibility of extra alignment markers (MM 7/29/18)
            if useExtra
               for iK = 1:size(extra,2)
                  nExt = length(extra{trials,iK});
                  nanSeparator = NaN(1,nExt);
                  exXPoints = [ extra{trials,iK} + relSpikeStartTime;...
                                extra{trials,iK} + relSpikeStartTime;...
                                nanSeparator ];
                  exXPoints = exXPoints(:);

                  exYPoints = [ (trials-halfSpikeHeight)*ones(1,nExt);...
                                (trials+halfSpikeHeight)*ones(1,nExt);...
                                 nanSeparator ];
                  exYPoints = exYPoints(:);
                  
                  ex_xPoints{iK}(ex_currentInd(iK):ex_currentInd(iK)+nExt*3-1) = exXPoints;
                  ex_yPoints{iK}(ex_currentInd(iK):ex_currentInd(iK)+nExt*3-1) = exYPoints;
                  
                  ex_currentInd(iK) = ex_currentInd(iK) + nExt*3;
               end
            end
            % (END)
      ...
      plot(plotH,xPoints, yPoints, 'k', lineFormat{:});
      
      % Additionally, plot other events (if desired; MM 7/29/18)
      if useExtra && strcmpi(plotType,'vertline')
         set(plotH,'NextPlot','add');
         ext_colors = load('spikeRaster_Colormap.mat','c');
         col_idx = round(linspace(1,size(ext_colors.c,1),size(extra,2)));
         
         for iK = 1:size(extra,2)
            plot(plotH,ex_xPoints{iK},ex_yPoints{iK},...
                 'Color',ext_colors.c(col_idx(iK),:),...
                 'LineWidth',2,...
                 'Marker','none');
         end
         
         if useExtraLabel
            extraLabel = reshape(extraLabel,numel(extraLabel),1);
            legend([{'spikes'}; extraLabel]);
         end
      end
