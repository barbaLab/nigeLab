...
%% Check on refrTime
minTime = pars.RefrTime * pars.fs *1e-3;
exclRefrTimeIDX = find(diff(ts) < minTime);
exclRefrTime = [];
for pp=exclRefrTimeIDX
    [~,minIdx]=min(abs(p2pamp([pp,pp+1])));
    exclRefrTime = [exclRefrTime minIdx-1+pp];
end
exclRefrTimeIDX = false(1,length(ts));
exclRefrTimeIDX(exclRefrTime) = true;
%% EXCLUDE VALUES OF PMAX <= 0
pm_ex = pmax<= 0 | pmin >= 0 | exclRefrTimeIDX;
ts(pm_ex) = [];
p2pamp(pm_ex) = [];
pmax(pm_ex) = [];
pmin(pm_ex) = [];
